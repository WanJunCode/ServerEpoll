...
void * AcceptThread(void *arg)
{
    ...
    unsigned short uListenPort = 12345;
    int iBacklogSize = 5;
                ...
                if (ret == 14 && msg.fd > 0)
                {
                    if (msg.op == 1)    //收到新的连接
                    {
                        ...
                        else
                        {
                            ...
                            printf("msg.fd = [%d] , tIpPort address = [%p] and mIpPort[msg.fd] address = [%p]\n",msg.fd,&tIpPort,&mIpPort[msg.fd]);
                            tPeerInfo.fd = msg.fd;
        ...
        for (i = 0; i < nfds && g_bRun; ++i)         
        {
            ...
            {        
                do
                {
                    ...
                    if (nread > 0)    //读到数据
                    {
                        cout << "ReadThread, read:" << nread<<", message:"<<buf << " ,errno:" << errno << endl;
